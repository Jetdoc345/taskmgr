...
BACKEND_URL = "http://127.0.0.0:5000/tasks"
...
def create_task():
    ...
    return (
        render_template("error.html", ststus=response.status_code),
        response.status_code
    )
...
def delete_tasks(pk):
    ...
    if response.status_code == 204:
        return render_template("success.htm", message="Task successfully deleted")
    return (
        render_template("error.html", status=response.status_code),
        response.status_code
    )